...
const publicRoute = computed(
  () => route.path === "/setup" || route.path === "/login",
);
const links = [
...
const pageName = computed(
  () =>
    links.find((link) => link.to !== "/" && route.path.startsWith(link.to))
      ?.label || (route.path === "/" ? "Visão geral" : "StackHost"),
);
function closeMenu() {
  open.value = false;
}
    ...
    <main>
      <header>
        ...
        <div class="breadcrumb">
          <span>StackHost</span><b>/</b><strong>{{ pageName }}</strong>
        </div>
        <span class="environment-chip"><span class="dot" /> Ambiente local</span
        ><RouterLink
